...
async function parseAmount(fileUrl) {
  try {
...
async function main() {
  ...
  const contracts = await prisma.document.findMany({
    ...
    select: { id: true, name: true, url: true, amount: true }
  });
  ...
  for (const doc of contracts) {
    ...
    if (amount !== null) {
      ...
      console.log(`   ✅ Banco atualizado: R$ ${amount}\n`);
    } else {
      console.log(`   ⚠️  Valor não encontrado, mantendo atual\n`);
    }
  }
