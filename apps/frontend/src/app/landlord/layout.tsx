...
export default function LandlordLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const roleStr = role || '';
    const hasAccess = roleStr.includes('LANDLORD') || roleStr.includes('OWNER') || roleStr.includes('ADMIN');
  ...
  const navItems = [
    { label: 'Meus Imóveis', icon: Home, href: '/landlord/dashboard' },
    { label: 'Documentos', icon: FolderOpen, href: '/landlord/documents' },
    { label: 'Suporte', icon: MessageCircle, href: '/landlord/support' },
  ];
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans selection:bg-primary selection:text-black">
      {/* Sidebar for Desktop - Black Identity */}
      <aside className="hidden md:flex flex-col w-72 h-screen fixed left-0 top-0 bg-black border-r border-white/5 z-40 overflow-hidden">
        <div className="p-10 border-b border-white/5 flex flex-col items-center gap-6">
           ...
           <div className="text-center space-y-1.5">
             <span className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-500">Proprietário SLX</span>
      ...
      <div className="flex-1 md:ml-72 flex flex-col min-h-screen bg-white">
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-slate-100 p-5 flex items-center justify-between md:hidden">
           <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-[9px] font-bold uppercase tracking-widest text-slate-400">Proprietário</span>
        ...
        <div className="hidden md:flex items-center justify-between px-12 py-10">
           <div className="space-y-1.5">
              <h1 className="text-4xl font-extrabold text-slate-900 tracking-tighter uppercase leading-none">
                {pathname === '/landlord/dashboard' ? 'Meus Imóveis' : 
                 pathname === '/landlord/documents' ? 'Documentos' : 
                 pathname === '/landlord/support' ? 'Suporte' : 'Portal'}
              </h1>
              <p className="text-slate-400 font-bold tracking-wide text-xs uppercase opacity-70">Gestão completa para proprietários SLX</p>
           </div>
           <div className="flex items-center gap-8">
              <div className="flex flex-col items-end">
                 <span className="text-slate-900 font-black text-sm tracking-tight">{userName}</span>
                 <span className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Área do Proprietário</span>
              </div>
